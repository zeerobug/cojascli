...
class chartSerie {
  ...
  get() {
    ...
    if (this.options.sort) {
      this.points = chartUtils.chartSort(
        this.points,
        this.options.order,
        this.options.direction
      );
    }
    if (_.isObject(this.options.limitedPoints)) {
      this._limitPoints(this.options.limitedPoints);
    }
    // TODO
  ...
  _preparePoints() {
    ...
    for (point of this.points) {
      ...
      if (point.x && (point.x instanceof Date || point.x._isAMomentObject)) {
  ...
  _limitPoints(options) {
    if (this.points.length >= options.max) {
      let remaining = this.points.slice(options.max);
      this.points = this.points.slice(0, options.max);
      let total = remaining.reduce((a, b) => a + b['y'], 0);
      console.log(total);
      let sup = remaining[0];
      sup.x = options.name;
      sup.label = options.name;
      sup.y = total;
      this.points.push(sup);
    }
  }
}
