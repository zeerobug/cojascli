...
class chartSerie {
  ...
  _setDataPoint(dataPoint) {
    ...
    if(dataPoint['label'] instanceof Date || dataPoint['label']._isAMomentObject) {
      if(dataPoint['label'] instanceof Date){
        dataPoint['label'] = moment(dataPoint['x']);
      }
      let format = this.opts.dateFormat ? this.opts.dateFormat : this.defaultdateFormat;
      dataPoint['label'] = dataPoint['label'].format(format);
    }
